...
fn main() {
    let options = eframe::NativeOptions {
        initial_window_size: Some(egui::vec2(680.0, 600.0)),
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.vertical_centered(|ui| {
                if !running {
                    ...
                    ui.add_space(5.0);
                    ui.label(
                        egui::RichText::new("Трафик идёт через web.telegram.org — провайдер видит обычный HTTPS")
            ...
            if running {
                ui.horizontal(|ui| {
                    ...
                    ui.label(
                        egui::RichText::new("(откроет Telegram, нажмите \"Подключить\")")
                            .color(ui.visuals().widgets.noninteractive.fg_stroke.color),
                    );
                });
                ui.add_space(8.0);
